...
export default function Footer() {
  return (
    <footer className="bg-[#020617] text-gray-300 border-t border-white/5 relative overflow-hidden py-24">
      {/* Subtle top glow */}
      ...
      <div className="mx-auto max-w-7xl px-6 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 gap-16 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="lg:col-span-1">
            <Link href="/" className="inline-block mb-10" aria-label="Kuno Lapidary Home">
              <Logo size="sm" />
            </Link>
          ...
          <div>
            <h3 className="text-[10px] font-bold uppercase tracking-[0.3em] text-gray-300 mb-10">
              Quick Links
            </h3>
            <ul className="space-y-4">
              {quickLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-gray-500 transition-colors duration-300 hover:text-teal-400 font-light"
                  >
                    {link.label}
                  </Link>
                </li>
            ...
            <div className="space-y-4">
              {legalLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="block text-sm text-gray-500 transition-colors duration-300 hover:text-teal-400 font-light"
                >
                  {link.label}
                </Link>
              ))}
        ...
        {/* Bottom Bar */}
        <div className="mt-24 pt-8 border-t border-white/5 flex flex-col items-center justify-between gap-6 text-[10px] font-bold uppercase tracking-[0.2em] text-gray-600 sm:flex-row">
